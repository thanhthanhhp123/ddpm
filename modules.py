...
class EMA:
    ...
    def update_average(self, old_weight, up_weight):
        if old_weight is None:
            return up_weight
        return old_weight * self.beta + (1 - self.beta) * up_weight
...
class Up(nn.Module):
    def __init__(self, in_channels, out_channels, emb_dim = 256):
        ...
        self.conv = nn.Sequential(
            DoubleConv(in_channels, out_channels, in_channels, residual=True),
            DoubleConv(out_channels, out_channels, in_channels //2)
        )
...
class UNet(nn.Module):
    def __init__(self, c_in = 3, c_out = 3, time_dim = 256, device = 'cuda' if torch.cuda.is_available() else 'cpu'):
        ...
        self.sa1 = SelfAttention(128, 28)
        self.down2 = Down(128, 256)
        self.sa2 = SelfAttention(256, 14)
        self.down3 = Down(256, 256)
        self.sa3 = SelfAttention(256, 7)
